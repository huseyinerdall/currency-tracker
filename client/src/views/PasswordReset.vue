      ...
      <v-row>
        <v-col class="mx-auto col-sm-10 col-xs-12 col-lg-5">
          ...
          <v-btn
            ...
            tile
            @click="sendPasswd"
            :loading="$store.state.isEmailSending"
            >GEÇİCİ PAROLA GÖNDER</v-btn
          >
        </v-col>
      </v-row>
    </v-container>
    ...
    >
      ...
      >
        <div style="width: 50%;" class="mx-auto">
          <v-text-field type="password" dark v-model="pin" label="Geçici şifre">
          </v-text-field>
          <v-text-field type="text" dark v-model="passwd1" label="Parola">
          </v-text-field>
          <v-text-field
            type="password"
            dark
          ...
          <v-btn @click="changePasswd" :loading="changing">
            Parola Değiştir
          </v-btn>
        </div>
      </v-container>
...
export default {
  ...
  methods: {
    sendPasswd() {
      ...
      if (this.email == "") {
        this.$toasted.error(`Alan boş bırakılamaz!`, {
          fullWidth: true,
          icon: "error",
          duration: 2000
        });
      ...
      axios
        ...
        .then(response => {
          if (response.data == "GOOGLEUSER") {
            this.$toasted.error(
              "Google ile giriş yapanlar parola değiştiremez!",
              { fullWidth: true, icon: "error", duration: 2000 }
            );
            this.$store.commit("isEmailSending", false);
            return;
          }
          if (response.data == "THEREISNOUSER") {
            this.$toasted.error("Böyle bir kullanıcı yok!", {
              fullWidth: true,
              icon: "error",
              duration: 2000
            });
          ...
          if (response.data == "MAILOK") {
            this.$store.commit("isEmailSending", false);
            this.$toasted.success(`Mail adresinize geçici parola gönderildi.`, {
              fullWidth: true,
              icon: "success",
              duration: 2000
            });
      ...
      axios
        ...
        .then(response => {
          if (response.data == "PINERROR") {
            this.$toasted.error("Geçici şifre hatalı!", {
              fullWidth: true,
              icon: "error",
              duration: 2000
            });
          ...
          if (response.data == "SHORT") {
            this.$toasted.error("En az 8 karakter veya rakam gerekli!", {
              fullWidth: true,
              icon: "error",
              duration: 2000
            });
          ...
          if (response.data == "NOTSAME") {
            this.$toasted.error("Parolalar uyuşmuyor!", {
              fullWidth: true,
              icon: "error",
              duration: 2000
            });
          ...
          if (response.data == "CHANGE") {
            ...
            this.$toasted.success(
              `Parolanız başarılı bir şekilde değiştirildi.`,
              { fullWidth: true, icon: "check", duration: 2000 }
            );
            this.resetFields();
