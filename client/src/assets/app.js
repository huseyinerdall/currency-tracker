...
const currencyXmlBodyStr = fs.readFileSync(
  "./static/altinkaynakCurrency.xml",
  "utf8"
);
    ...
    .then(response => {
      ...
      parseString(xml, function(err, result) {
        ...
        for (let i = 0; i < result.length; i++) {
          ...
          temp["sell"] =
            result[i]["BanknoteSelling"][0] || result[i]["ForexSelling"][0];
          temp["buy"] =
            result[i]["BanknoteBuying"][0] || result[i]["ForexBuying"][0];
          factRes.push(temp);
        }
  ...
  axios
    ...
    .then(response => {
      for (const element in response.data) {
        if (
          api[element].indexOf("Altın") > 0 ||
          api[element] == "22 Ayar Bilezik" ||
          api[element] == "Gümüş"
        ) {
...
app.get("/gold/:goldName", (req, res) => {
  ...
  axios
    ...
    .then(async response => {
      let sepetalis = (
        (parseFloat(response.data["USD"]["Alış"]) +
          parseFloat(response.data["EUR"]["Alış"])) /
        2
      ).toFixed(4);
      let sepetsatis = (
        (parseFloat(response.data["USD"]["Satış"]) +
          parseFloat(response.data["EUR"]["Satış"])) /
        2
      ).toFixed(4);
      response.data["SEPET KUR"] = { Alış: sepetalis, Satış: sepetsatis };
      response.data[element]["time"] = response.data["Update_Date"];
      if (
        element.indexOf("altin") > 0 ||
        element == "22-ayar-bilezik" ||
        element == "gumus"
      ) {
        ...
        a = await db[indis]
          .findOne({
            where: {
              createdAt: {
                [Op.between]: [
                  moment(date + " " + prevTime).toDate(),
                  moment(date + " " + nextTime).toDate()
                ]
              }
            }
          })
        ...
        element.indexOf("Update") < 0 &&
        element.indexOf("ÇEKME") < 0
      ) {
        response.data[element]["type"] = api[element];
...
app.get("/currencies", (req, res) => {
  axios
    .post(
      "http://data.altinkaynak.com/DataService.asmx",
      currencyXmlBodyStr,
      config
    )
    .then(response => {
      ...
      parseString(xml, function(err, result) {
        parseString(
          result["soap:Envelope"]["soap:Body"][0]["GetCurrencyResponse"][0][
            "GetCurrencyResult"
          ][0],
...
app.get("/coins", (req, res) => {
  ...
  axios
    .get(
      `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=550&page=1&sparkline=false&price_change_percentage=24h,7d`
    )
    .then(response => {
      for (let i = 0; i < response.data.length; i++) {
        ...
        temp["close"] =
          parseFloat(response.data[i]["current_price"]) -
          parseFloat(response.data[i]["price_change_24h"]); //close price
        factRes.push(temp);
      }
      res.json(factRes);
    });
...
app.post("/getcoinaccordingtotimerange", async (req, res) => {
  ...
  if (time == 1) {
    ...
    data = await db.sequelize.query(
      `select distinct on ("createdAt"::date) * from "${coinSymbol.toUpperCase()}s" where "createdAt" between '${BEGIN.toISOString()}' and '${NOW.toISOString()}'`,
      null,
      {
        raw: true
      }
...
app.post("/getgoldaccordingtotimerange", async (req, res) => {
  ...
  try {
    if (time == 1) {
      ...
      data = await db.sequelize.query(
        `select distinct on ("createdAt"::date) * from "${"Gold" +
          utils.turkishToEnglish(
            goldName
          )}s" where "createdAt" between '${BEGIN.toISOString()}' and '${NOW.toISOString()}'`,
        null,
        {
          raw: true
        }
    ...
    if (time == 1) {
      ...
      data = await db.sequelize.query(
        `select distinct on ("createdAt"::date) * from "${utils.turkishToEnglish(
          goldName
        )}s" where "createdAt" between '${BEGIN.toISOString()}' and '${NOW.toISOString()}'`,
        null,
        {
...
app.post("/bintltable", async (req, res) => {
  ...
  axios
    ...
    .then(response => {
      ...
      BINTL["GoldGramAltin"] = parseFloat(
        response.data["gram-altin"]["Satış"].replace(",", ".")
      );
      BINTL["GoldOnsAltin"] = parseFloat(
        response.data["ons"]["Satış"].replace(",", ".")
      );
      BINTL["GoldGumus"] = parseFloat(
        response.data["gumus"]["Satış"].replace(",", ".")
      );
      axios
        .get(
          "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin,ethereum,ripple,dogecoin,tether&order=id_asc&per_page=100&page=1&sparkline=false&price_change_percentage=24h,7d"
        )
        .then(async response => {
          ...
          for (let i = 0; i < BINTLTABLE_LIST.length; i++) {
            ...
            if (
              !(
                BINTLTABLE_LIST[i].length == 3 ||
                BINTLTABLE_LIST[i] == "DOGE" ||
                BINTLTABLE_LIST[i] == "USDT"
              )
            ) {
              ...
              temp["value"] =
                (BINTL[BINTLTABLE_LIST[i]] -
                  parseFloat(a["dataValues"]["Satis"].replace(",", "."))) *
                (1000 / BINTL[BINTLTABLE_LIST[i]]);
            } else {
              a = await db[BINTLTABLE_LIST[i]].findOne({
                where: {
                  createdAt: {
              ...
              temp["type"] = BINTLTABLE_LIST[i];
              temp["value"] =
                (+BINTL[BINTLTABLE_LIST[i]] - a["dataValues"]["Fiyat"]) *
                (1000 / (BINTL[BINTLTABLE_LIST[i]] * BINTL["ABDDOLARI"]));
            }
            data.push(temp);
          }
          res.json(data);
    ...
    .then(async response => {
      ...
      for (const element in response.data) {
        ...
        if (
          ...
          api[element].indexOf("Update") < 0 &&
          api[element].indexOf("ÇEKME") < 0
        ) {
...
app.post("/buynow", (req, res) => {
  ...
  Trader.setBuyOrder(
    ...
    Trader.buy(userId, wealth, parameter, amount, "TÜRK LİRASI", orderId);
  });
...
app.post("/sellnow", (req, res) => {
  ...
  Trader.setSellOrder(
    ...
    Trader.sell(userId, wealth, parameter, amount, "TÜRK LİRASI", orderId);
  });
...
app.post("/setsellorder", (req, res) => {
  ...
  let amount = req.body.amount;
  let major = req.body.major || "TÜRK LİRASI";
  let result = Trader.setSellOrder(
    userId,
    orderType,
    parameter,
    wealth,
...
db.sequelize.sync().then(() => {
  ...
  setInterval(() => {
    ...
    axios
      .get(
        `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=markey_cap_desc&per_page=250&page=1&sparkline=true&price_change_percentage=24h,7d`
      )
      .then(response => {
        for (let i = 0; i < response.data.length; i++) {
          temp = {};
          ...
          temp["pricechange24h"] =
            response.data[i]["price_change_percentage_24h"];
          temp["pricechange7d"] =
            response.data[i]["price_change_percentage_7d_in_currency"] || 0;
          temp["time"] = response.data[i]["last_updated"];
          temp["image"] = response.data[i]["image"];
          temp["sparkline"] = response.data[i]["sparkline_in_7d"][
            "price"
          ].slice(0, 20);
          allPrices[response.data[i]["name"]] = -response.data[i][
            "current_price"
          ];
          factRes.push(temp);
          if (i < 30) {
          ...
          if (
            response.data[i]["last_updated"] != M[response.data[i]["symbol"]] &&
            db[response.data[i]["symbol"].toUpperCase()]
          ) {
            if (response.data[i]["symbol"].toUpperCase() == "1INCH") {
              response.data[i]["symbol"] = "ONEINCH";
            }
            if (
              response.data[i]["symbol"].toUpperCase() == "YVAULT-LP-YCURVE"
            ) {
              response.data[i]["symbol"] = "YVAULTLPYCURVE";
            }
            db[response.data[i]["symbol"].toUpperCase()].create({
              Fiyat: response.data[i]["current_price"]
            });
      ...
      .then(async response => {
        ...
        for (const element in response.data) {
          ...
          if (
            ...
            if (
              (db[indis] && response.data[element]["Alış"] != C[indis]) ||
              (new Date().getHours() == "1" &&
                new Date().getMinutes() == "20" &&
                new Date().getSeconds() < 15)
            ) {
              db[indis].create({
                Alis: response.data[element]["Alış"],
                Satis: response.data[element]["Satış"]
              });
            ...
            api[element].indexOf("Update_Date") < 0 &&
            api[element].indexOf("ÇEKME") < 0
          ) {
            response.data[element]["type"] = api[element];
            response.data[element]["time"] = updatetime;
            ...
            allPrices[api[element]] = parseFloat(
              response.data[element]["Satış"]
            );
          }
        }
      })
  ...
  setInterval(() => {
    if (factRes.length == 250) {
      io.emit("coins", factRes);
    }
    if (factRes30.length == 30) {
  ...
  setInterval(() => {
    let openOrders = Trader.getAllOpenOrders().then(openOrders => {
      ...
      for (let i = 0; i < openOrders.length; i++) {
        if (openOrders[i]["dataValues"]["OrderType"] === "price") {
          if (
            openOrders[i]["dataValues"]["buyOrSell"] == "buy" &&
            parseFloat(
              openOrders[i]["dataValues"]["Parameter"].replace(",", ".")
            ) >=
              parseFloat(
                allPrices[openOrders[i]["dataValues"]["CoinOrCurrency"]] < 0
            ...
            openOrders[i]["dataValues"]["buyOrSell"] == "sell" &&
            parseFloat(
              openOrders[i]["dataValues"]["Parameter"].replace(",", ".")
            ) <=
              parseFloat(
...
setTimeout(() => {
  THE_BEGINNING_OF_EVERYTHING = false;
}, 20000);
