      ...
      <v-container style="border: 1px solid #ddd;border-radius:0;position:relative;"
        ...
        <v-row class="justify-space-between">
          <v-col cols="10">
            ...
            <h4 class="white--text">{{avaibleBalance | turkishCurrencyformat}} ₺</h4>
          </v-col>
          <v-spacer></v-spacer>
          <v-col>
            <v-btn icon :disabled="!walletLoaded" @click="getUserWallet">
              <v-icon color="white" :class="[walletLoaded ? '' : 'mdi-spin']">mdi-reload</v-icon>
            </v-btn>
          </v-col>
        </v-row>
        <v-data-table
        ...
        >
          <template v-slot:item="{ item }">
            <tr v-if="+item.amount>0">
              <td class="text-uppercase white--text" :style="[$store.state.isLight ? 'color:rgba(0,0,0,0.87) !important;' : 'color:#ffffff !important;']">{{item.shortName}}</td>
              <td class="text-right white--text" :style="[$store.state.isLight ? 'color:rgba(0,0,0,0.87) !important;' : 'color:#ffffff !important;']">{{ item["amount"] | turkishCurrencyformat}}</td>
            </tr>
          </template>
        </v-data-table>
      </v-container>
        ...
        id: JSON.parse(localStorage.getItem('user')).id
          ...
          .then((response) => {
            ...
            for (const wealth in response.data) {
              ...
              this.balance += response.data[wealth].amount * ((allPrices[wealth]<0 ? this.dolar*allPrices[wealth]*-1 : allPrices[wealth])||0);
